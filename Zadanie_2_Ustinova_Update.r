...
tbl = filter(tbl, month %in% c("Март","Апрель","Май"))
...
cor_td = cor(tbl_numeric)%>% as.data.frame %>% select(qc_h2o_flux)
...
varsCor1 = row.names(cor_td)[cor_td$qc_h2o_flux^2 > .1] %>% na.exclude
varsCor1

# запись параметров с кореляцией >0,1 для модели регрессионного анализа
formula1 = as.formula(paste("qc_h2o_flux~", paste(varsCor1,collapse = "+"), sep=""))
formula1

# запись всех параметров  для второй модели регрессионного анализа
formula2 = as.formula(paste("qc_h2o_flux~", paste(varsCor2,collapse = "+"), sep=""))
formula2
